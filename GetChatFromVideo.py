...
videoList = ['601206746']  #, '355484291', '355946365', '356914013'
...
for videoId in videoList:
    ...
    # ==========================================================================
    # получаем сообщения (комменты) чата видео
    h = {STR_ACCEPT: HEADER_AcceptV5,
         STR_CLIENT: clientId}
    ...
    with open(outputFilename + '.txt', 'wb') as chatfile:
        while canProduce:
            if 'comments' in j:
                msgCount += len(j['comments'])
                for msg in j['comments']:
                    s_msgTime = msg['created_at']
                    s_msgText = msg['message']['body']

                    # обрабатываем смайлы (подсчитываем)
                    for fragment in msg['message']['fragments']:
                        if 'emoticon' in fragment:
                            smile = fragment['text']
                            if smile in d_smilesCount:
                                d_smilesCount[smile] += 1
                            else:
                                d_smilesCount[smile] = 1

                    s_msgType = msg['source']
                    s_msgNickname = msg['commenter']['display_name']
                    # обрабатываем пользователей (подсчитываем)
                    if s_msgNickname in d_chattersCount:
                        d_chattersCount[s_msgNickname] += 1
                    else:
                        d_chattersCount[s_msgNickname] = 1

                    i_timeRelative = msg['content_offset_seconds']

                    s_msgTime = s_msgTime[:-1].split('.')[0]  # 2017-11-13T13:00:17.710599Z -> 2017-11-13T13:00:17Z
                    ...
                    srch = re.search('(?i)^([^\S]?(LUL|OMEGAROLL|hyperWOW|WLGW|POG|PogChamp|WutFace|MLG|МЛЖ|МЛГ|млг|ЭмЭлДжи|DansGame|SeemsGood|CurseLit|GTChimp|SwiftRage|CarlSmile|CoolStory|pepeLaugh|KEKW|dangarLol|BSUTrolled)[\s]*)+$|^([\s]?[\S]+LUL[\s]?)+|^([\s]?[\S]+MLG[\s]?)+|^([\s]?[\S]+RAGE[\s]?)+|^[ах]{3,}\)*[\s]*$|^[X:;]D*\)*[\s]*$|^хд*\)*[\s]*$', s_msgText, re.IGNORECASE)
    ...
    with open(momentsFilename, 'w', encoding='utf8') as wowFile:
        ...
        if l_clips:
            wowFile.write('\n = КЛИПЫ = \n')
            # Делаем множество, чтобы убрать повторяющиеся
            set_slugs = set()
            for clip in l_clips:
            ...
            for slug in set_slugs:
                ...
                strOffset = '%s%s%02d' % (hs, ms, sec)
                writestr = f'https://clips.twitch.tv/{slug} ({clipAuthor}) ({strOffset} - {clipDuration})\n'
                wowFile.write(writestr)

            # А это ну просто лист чата, когда кидали клипы
            wowFile.write('\n---------\n = КЛИПЫ КАК В ЧАТЕ = \n')
            for clip in l_clips:
                wowFile.write(f'{clip}\n')
        ...
        wowFile.write('\n---------\n')
        wowFile.write(f'Всего сообщений: {msgCount}\n')
        wowFile.write(f'Всего чаттеров: {len(d_chattersCount)}\n')
        wowFile.write(f'ТОП-5 чаттеров:\n')
        list_d = list(d_chattersCount.items())
        list_d.sort(reverse=True, key=lambda i: i[1])
        c = 0
        for chatter in list_d:
            wowFile.write(f'\t{chatter[0]}: {chatter[1]}\n')
            c += 1
            if c == 5:
                break
        wowFile.write(f'Всего смайлов: {len(d_smilesCount)}\n')
        wowFile.write(f'ТОП-10 смайлов:\n')
        list_d = list(d_smilesCount.items())
        list_d.sort(reverse=True, key=lambda i: i[1])
        c = 0
        for smile in list_d:
            wowFile.write(f'\t{smile[0]}: {smile[1]}\n')
            c += 1
            if c == 10:
                break
    ...
    cmdline = f'"C:\\Program Files\\Sublime Text 3\\sublime_text.exe" "{momentsFilename}"'
